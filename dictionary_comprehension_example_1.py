...
def generate_dictionary_one_liner() -> dict:
    dictionary = {f"{i}": f"{"FizzBuzz" if i%5==0 else "Fizz"}" for i in range(1,201) if i%3==0 and i%4==0}
    return dictionary

def generate_dictionary_one_liner_2() -> dict:
    dictionary = {f"{i}": f"{"FizzBuzz" if i%5==0 else "Fizz"}" for i in range(0,201,12)}
    return dictionary
